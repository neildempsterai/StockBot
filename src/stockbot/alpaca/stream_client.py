...
class StreamClient:
    ...
    def get_subscribed(self) -> list[str]:
        """Return current subscription list (for gateway refresh comparison)."""
        return sorted(self._subscribed)

    def set_subscriptions(self, symbols: list[str]) -> None:
        """Replace subscription set (e.g. before reconnect)."""
        self._subscribed = set(s.strip() for s in symbols if s and str(s).strip())
